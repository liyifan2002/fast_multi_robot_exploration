...
void MapROS::processDepthImage() {
  ...
  if (depth_pub_.getNumSubscribers() > 0) publishDepth();
}
...
void MapROS::publishFree() {
  ...
  if (free_pub_.getNumSubscribers() == 0) return;

  pcl::PointXYZ pt;
  pcl::PointCloud<pcl::PointXYZ> unknown, free;
  Eigen::Vector3i min_idx, max_idx;
  map_->posToIndex(map_->md_->all_min_, min_idx);
  map_->posToIndex(map_->md_->all_max_, max_idx);
  ...
  const double visualization_truncate_height = 2.09;
  const double visualization_truncate_low = -0.09;
    ...
    for (int y = min_idx(1); y <= max_idx(1); ++y)
      for (int z = min_idx(2); z <= max_idx(2); ++z) {
        if (map_->md_->occupancy_buffer_[map_->toAddress(x, y, z)] <=
          ...
          Eigen::Vector3d pos;
          map_->indexToPos(Eigen::Vector3i(x, y, z), pos);
          if (pos(2) > visualization_truncate_height) continue;
          if (pos(2) < visualization_truncate_low) continue;
          pt.x = pos(0);
          pt.y = pos(1);
          pt.z = pos(2);
          free.push_back(pt);
  ...
  // Publish free
  free.width = free.points.size();
  free.height = 1;
  free.is_dense = true;
  free.header.frame_id = frame_id_;
  sensor_msgs::PointCloud2 free_msg;
  pcl::toROSMsg(free, free_msg);
  free_pub_.publish(free_msg);
}
